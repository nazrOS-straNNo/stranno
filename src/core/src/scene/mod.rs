...
impl SceneGraph {
    ...
    pub fn привязать(
        ...
        if let Some(узел) = self.узлы.get_mut(&дочерний) {
            // Убрать из старого родителя / корней
            if let Some(старый_родитель) = узел.родитель {
                if let Some(р) = self.узлы.get_mut(&старый_родитель) {
                    р.дети.retain(|&c| c != дочерний);
                }
            } else {
                self.корни.retain(|&r| r != дочерний);
            }
            узел.родитель = Some(родитель);
        } else {
            return Err(CoreError::УзелНеНайден(дочерний));
        }

        if let Some(р) = self.узлы.get_mut(&родитель) {
            р.дети.push(дочерний);
        }
